...
public class InterstitialVideoAd 
{
	...
	public InterstitialVideoAd(Activity activity, String key)
	{
		init(activity, key, null);
	}
	
	public InterstitialVideoAd(Activity activity, String key, AdLocusTargeting adLocusTargeting)
	{
	...
	private void initViews()
	{
		...
		final WebView webView = new WebView(mActivity)
		{
			...
			public void loadUrl(final String url)
			{
				...
				{
					super.loadUrl(url); //for javascript
					return;
				}
									...
									synchronized (mIsLoading)
									{
										...
										synchronized (mIsReadyToShow)
										{
											...
											if(mListener != null)
											{
												...
												{
													case 404:
														mListener.onFailedToReceiveAd(InterstitialVideoAd.this, ErrorCode.NO_FILL);
														break;
													case 403:
														mListener.onFailedToReceiveAd(InterstitialVideoAd.this, ErrorCode.INVALID_KEY);
														break;
													default:
														if(errorCode > 500)
														{
															mListener.onFailedToReceiveAd(InterstitialVideoAd.this, ErrorCode.SERVICE_ERROR);
														}
														else
														{
															mListener.onFailedToReceiveAd(InterstitialVideoAd.this, ErrorCode.NETWORK_ERROR);
														}
														break;
												}
											}
					...
					public void run()
					{
						synchronized (mIsLoading)
						{
							synchronized (mIsReadyToShow)
							{
								...
								if(mListener != null)
								{
									mListener.onReceiveAd(InterstitialVideoAd.this);
								}
							}
						}
					}
        ...
        webView.setWebViewClient(new WebViewClient()
        {
			...
			{			
				...
				else
				{
					...
					InterstitialAdListener listener = mListener;
					...
					synchronized (mIsLoading)
					{
						...
						synchronized (mIsReadyToShow)
						{
							...
							if (listener != null)
							{
								listener.onFailedToReceiveAd(InterstitialVideoAd.this, ErrorCode.NETWORK_ERROR);
							}
						}
					}
					return true;
			...
			{
				com.hyxen.adlocusaar.util.Log.d(url);

				if(view.getHitTestResult() != null
						&& view.getHitTestResult().getType() > 0 
						&& !url.contains("/dev_html5/imp")
						&& !url.contains("/dev_html5/fscb")
		...
		synchronized (mIsLoading)
		{
			...
			synchronized (mIsReadyToShow)
			{
				...
				HxRequest r = new HxRequest(mActivity) {
					@Override
					public void run() {
						AdLocusUtil.setRequestParameters(mActivity, this, mKey, AdLocusUtil.getScreen(mActivity, AdLocusLayout.AD_SIZE_IAB_MRECT), mActivity.getLocalClassName(), mAdLocusTargeting);
						super.run();
					}
				};
