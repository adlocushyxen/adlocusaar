...
public class HxRequest implements Runnable
{
	...
	public HxRequest(Context context)
	{
		mContext = context;
	}
	...
	public void run()
	{
		...
		while(true) {
			...
			try
			{
				...
				if (!mPostParameters.containsKey("accept-charset")) mConnection.setRequestProperty("Accept-Charset", "UTF-8");
				if (!mPostParameters.containsKey("content-type")) mConnection.setRequestProperty("Content-Type", "application/x-www-form-urlencoded;charset=" + "UTF-8");
	...
	private String getUserAgent(){
		...
		if(mContext != null) userAgent += " " + mContext.getPackageName();
